...
public:
	...
	Item(Key k, string n, string e, int v) {
		matricola = k;
		fullName = n;
		email = e;
		voto = v;
	}
	Key key() { return matricola; }
	
	void show(ostream& os = cout)
	{
		os << matricola << "," << fullName << "," << voto << "," << email << endl;
	}

	int getvoto() { return voto; }
	string getnome() { return fullName; }
};
...
class BST 
  {
    private:
	  ...
	  void attraversamentoR(link h, BST<Item, Key>& b)
	  {
		  if (h == 0) return;

		  Item primo = h->item;
		  Item secondo = b.search(primo.key());

		  if (secondo.key()) {
			  float mediavoti = (float(primo.getvoto()) + float(secondo.getvoto())) / 2;
			  if (mediavoti >= 18) {
				cout << primo.getnome() << ", " << mediavoti << endl;
			  }
			  else {
				  cout << primo.getnome() << ", INS" << endl;
			  }
		  }
		  else {
			  if (primo.getvoto() >= 18) {
				cout << primo.getnome() << ", " << primo.getvoto() << endl;
			  }
			  else {
				  cout << primo.getnome() << ", INS" << endl;
			  }
		  ...
		  attraversamentoR(h->l, b);
		  attraversamentoR(h->r, b);
	  }
   ...
   */
	public:
	  ...
	  void attraversamento(BST<Item, Key>& b) {
		  attraversamentoR(head, b);
	  }
