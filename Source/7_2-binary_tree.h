	...
	void traverse_levelorder_NR(Node<T>* theRoot)
     ...
     int equal_structure(Node<T>* theRootA, Node<T>* theRootB) {
         if (theRootA == NULL) {
             ...
             if (theRootB == NULL) return 0;
             //qui sotto theRootA e theRootB non sono NULL
             int temp = equal_structure(theRootA->lChildptr, theRootB->lChildptr);
             if (temp == 0) return 0;
             temp = equal_structure(theRootA->rChildptr, theRootB->rChildptr);
             if (temp == 0) return 0;
         }
